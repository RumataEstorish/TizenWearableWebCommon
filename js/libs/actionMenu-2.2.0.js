 ...
 * 2.1.2
 * fixed possible error when open
 * 2.1.1.1
 ...
 *            link when click event, title : display name of menu item, noBack: don,t return to caller page, navigateTo: navigate to desired page}
 */
function ActionMenu(page, menuName, menuItems) {
    var i = 0, _page = page[0] === "#" ? page : "#" + page, _menuName = menuName[0] === "#" ? menuName : "#" + menuName, prevPage = null, opened = false, clickBound = _elementClick.bind(null), selectorElement = null;
...
ActionMenu.prototype.showMenuItem = function (itemName) {
    ...
    if (popup.length) {
        ...
        menuItem.jContent.on('click', function () {
            self.close(function () {
                menuItem.onclick();
            }, menuItem.navigateTo, menuItem.noBack);
        });
    } else {
...
ActionMenu.prototype.show = function () {
        ...
        for (i = 0; i < self.menuItems.length; i++) {
            (function (i) {
                $("#" + self.menuItems[i].name).on("click", function () {
                    ...
                    }, self.menuItems[i].navigateTo, self.menuItems[i].noBack);
                });
            })(i);
    ...
    if (tau.support.shape.circle) {
        ...
        if (!this.selectorElement || this.selectorElement.length === 0) {
            return;
        }
        ...
        this.selectorElement.on("click", function (event) {
            ...
            var activeItem = $(event.target).parent().find(".ui-item-active").eq(0);
            var item = self.getMenuItemByName(activeItem.prop('id'));
            self.close(function () {
                item.onclick();
            }, item.navigateTo, item.noBack);
        });
    }
 ...
 * @param navigateTo - if set, navigate to page. If not set, got back to previous
 * @param noBack - if set, no navigate to previous page
 */
ActionMenu.prototype.close = function (onclosed, navigateTo, noBack) {
    var self = this;
    ...
    $(self.page).one('pagehide', function () {
        /*if (self.prevPage !== self.page && !navigateTo) {
            tau.changePage("#" + self.prevPage, {
                'transition': 'none'
            });
        }
        if (navigateTo) {
            tau.changePage("#" + navigateTo, {
                'transition': 'none'
            });
        }*/
        if (onclosed) {
